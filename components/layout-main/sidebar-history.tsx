...
import type { Chat } from "@/types/chat-related";
...
export function getChatHistoryPaginationKey(
  ...
  if (pageIndex === 0) {
    return `${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/api/chats?limit=${PAGE_SIZE}`;
  }
  ...
  return `${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/api/chats?ending_before=${firstChatFromPage.chatId}&limit=${PAGE_SIZE}`;
}

export function SidebarHistory({ user }: { user: UIUser | undefined }) {
    ...
    mutate((chatHistories) => {
      if (chatHistories) {
        return chatHistories.map((chatHistory) => ({
          ...chatHistory,
          chats: chatHistory.chats.filter((chat) => chat.chatId !== chatToDelete),
        }));
      }
    });
